...
export class Query {
  ...
  public getQueryKey(id: string): QueryKey {
  ...
  public getKeyNames(): Array<string> {
    ...
    if (struct.getDataStructureComponents().getDimensionList().getMeasureDimension() != null) {
      var dim2: structure.MeasureDimension = struct
  ...
  getUpdatedAfter(): Date {
    return this.updatedAfter;
  }
  setUpdatedAfter(d: Date) {
    this.updatedAfter = d;
  }
  ...
  setIncludeHistory(b: boolean) {
    this.includeHistory = b;
  }
  setProviderRef(s: string) {
    this.providerRef = s;
  }
...
export class QueryKey {
  ...
  public removeDuplicates(list: Array<structure.ItemType>): Array<structure.ItemType> {
    return list.reduce((acc, e) => (acc.includes(e.getId().toString()) ? [...acc, e] : acc), []);
  }
  public size() {
...
export class StructuredDataMessage {
  ...
  public buildStructuredDataSet(i: number): StructuredDataSet {
    ...
    var structure: structure.DataStructure = this.getRegistry().findDataStructure(
      this.getDataMessage().getHeader().getStructures()[0].getStructure(),
    );
  ...
  public static resolveCode(
    ...
    if (conceptRef != null) {
      ...
      if (concept == null) {
        ...
        console.log(
          conceptRef.getAgencyId() +
            ':' +
            conceptRef.getMaintainableParentId() +
            ':' +
            conceptRef.getId() +
            ':' +
            conceptRef.getVersion(),
        );
...
export class Cube {
  ...
  private validCodes: collections.Dictionary<string, Array<string>> = new collections.Dictionary<
    string,
    Array<string>
  >();
  ...
  public putObservation(order: Array<string>, mapper: interfaces.ColumnMapper, obs: FlatObs) {
    ...
    for (
      var i: number = 0;
      i < this.struct.getDataStructureComponents().getDimensionList().getDimensions().length;
      i++
    ) {
    ...
    var dimId: commonreferences.ID = this.struct
      .getDataStructureComponents()
      .getDimensionList()
      .getTimeDimension()
      .getId();
    ...
    var dimId3: commonreferences.ID = this.struct
      .getDataStructureComponents()
      .getMeasureList()
      .getPrimaryMeasure()
      .getId();
    ...
    for (
      var k: number = 0;
      k < this.struct.getDataStructureComponents().getAttributeList().getAttributes().length;
      k++
    ) {
      var name: string = this.struct
        .getDataStructureComponents()
        .getAttributeList()
        .getAttributes()
        [k].getId()
  ...
  public findLatestCubeObs(key: FullKey, latest: boolean): CubeObs {
      ...
      if (dim == null) {
        console.log(
          "Can't find dim:" +
            ...
            ':' +
            structure.NameableType.toIDString(key.getComponent(oldDim.getSubDimension())),
        );
        return null;
      }
      oldDim = dim;
    ...
    if (time == null) {
      ...
      if (this.struct.getDataStructureComponents().getDimensionList().getMeasureDimension() != null) {
        var measure: string = structure.NameableType.toIDString(
          key.getComponent(
            this.struct.getDataStructureComponents().getDimensionList().getMeasureDimension().getId().toString(),
          ),
        );
        //tcd.dump();
  ...
  public findLatestFlatObs(key: FullKey, latest: boolean): FlatObs {
    ...
    if (time == null) {
      ...
      if (this.struct.getDataStructureComponents().getDimensionList().getMeasureDimension() != null) {
        var measure: string = key.getComponent(
          this.struct.getDataStructureComponents().getDimensionList().getMeasureDimension().getId().toString(),
        );
...
export class CubeObservation {
  ...
  constructor(
    crossSectionalMeasureConcept: string,
    crossSection: string,
