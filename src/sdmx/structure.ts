...
export class MaintainableType extends VersionableType {
  ...
  identifiesMe(
    ...
    vers2: commonreferences.Version,
  ): boolean {
    /*
...
export class DimensionList {
  ...
  public getMeasureDimension(): MeasureDimension {
...
export class MeasureList {
  ...
  public getPrimaryMeasure(): PrimaryMeasure {
...
export class DataStructure extends MaintainableType {
      ...
      if (dim1.getLocalRepresentation().getEnumeration() != null) {
        console.log(
          'Dim:' +
            i +
            'enum ref:agency' +
            dim1.getLocalRepresentation().getEnumeration().getAgencyId() +
            ':mid' +
            dim1.getLocalRepresentation().getEnumeration().getMaintainableParentId() +
            ':' +
            dim1.getLocalRepresentation().getEnumeration().getId() +
            ':v:' +
            dim1.getLocalRepresentation().getEnumeration().getVersion(),
        );
      }
    }
      ...
      if (dim.getLocalRepresentation().getEnumeration() != null) {
        console.log(
          'Dim:' +
            'pm' +
            'enum ref:agency' +
            dim.getLocalRepresentation().getEnumeration().getAgencyId() +
            ':mid' +
            dim.getLocalRepresentation().getEnumeration().getMaintainableParentId() +
            ':' +
            dim.getLocalRepresentation().getEnumeration().getId() +
            ':v:' +
            dim.getLocalRepresentation().getEnumeration().getVersion(),
        );
      }
    }
  ...
  public isPrimaryMeasure(s: string): boolean {
    ...
    if (this.getDataStructureComponents().getMeasureList().getPrimaryMeasure().getId().toString() == s) {
      return true;
    }
    return false;
  }
...
export class Concepts {
  ...
  findConceptSchemeReference(ref: commonreferences.Reference): ConceptSchemeType {
    if (ref == null) {
      ...
      var cs: ConceptSchemeType = this.findConceptScheme(
        ref.getAgencyId(),
        ref.getMaintainableParentId(),
        ref.getVersion(),
      );
